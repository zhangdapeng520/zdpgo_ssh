...
import (
	"errors"
	"fmt"
	"github.com/zhangdapeng520/zdpgo_ssh/sftp/internal/encoding/ssh/filexfer"
	"os"
	"os/user"
	"strconv"
	"time"
...
func runLs(idLookup NameLookupFileLister, dirent os.FileInfo) string {
	...
	symPerms := filexfer.FileMode(fromFileMode(dirent.Mode())).String()

	var numLinks uint64 = 1
	uid, gid := "0", "0"

	switch sys := dirent.Sys().(type) {
	case *filexfer.Attributes:
